...
export class AuthWorker extends WorkerEntrypoint {
  ...
  async verifyAuth(oauthId, organization=null, properties=null) {
    ...
    if (organization) {
      if (Array.isArray(organization) && !organization.includes(user.organization)) return { auth: false, session: true };
      if (typeof organization === "string" && organization !== user.organization) return { auth: false, session: true };
    }
    // TODO: Add more config options (includes, number comparison, etc.)
    if (properties) {
      for (let key in properties) {
        if (!(key in user) || properties[key] !== user[key]) return { auth: false, session: true };
      }
    }
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "redirect") {
        ...
        if (!tokenResponse.ok) return new Response("Failed to exchange code for token: " + JSON.stringify(await tokenResponse.json()), { status: 401 });
      ...
      if (path === "logout") {
        ...
        if (login || customRedirectUrl) response.headers.set("Location", login ? (`https://workers.tablerus.es/auth/login` + customRedirectUrl ? ("?redirect=" + encodeURIComponent(customRedirectUrl)) : "") : customRedirectUrl);
        return response;
      }
